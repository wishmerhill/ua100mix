...
from fulleffectsdialog_ui import *
...
def setupMixer(ui,window):
    ...
    if (MIXER_OUTPUT_MODE):
        for key in MIXER_EFFECT_MODE_PAR.keys():
            ui.EffectModeSelector.addItem(MIXER_EFFECT_MODE_PAR[key], key)
        ui.EffectModeSelector.setCurrentIndex(-1)
    ui.EffectModeSelector.currentIndexChanged.connect(setEffectMode)
    
    ui.EffWave1Button.clicked.connect(functools.partial(effectSelection, ui))
    #ui.EffWave2Button.clicked.connect(setEff2)
    #ui.EffMic2Button.clicked.connect(effOn)
...
def effectSelection(ui):
    print ui.sender().getObjectName()

#def setEff1():
#    time.sleep(SLEEP_TIME)
#    send_DT1([0x00, 0x40, 0x03, 0x00, 0x00, 0x16])
#    time.sleep(SLEEP_TIME)
#    send_DT1([0x00,0x40,0x03, 0x03, 0x7f])
#    time.sleep(SLEEP_TIME)
#
#def setEff2():
#    time.sleep(SLEEP_TIME)
#    send_DT1([0x00, 0x40, 0x05, 0x00, 0x00, 0x16])
#    time.sleep(SLEEP_TIME)
#    send_DT1([0x00,0x40,0x05, 0x03, 0x00])
#    time.sleep(SLEEP_TIME)
#    
def effOn():
    time.sleep(SLEEP_TIME)
    send_DT1([0x00, 0x40, 0x40, 0x05, 0x01])
...
def setUpFullEffectDialog(self):
    '''
...
fullEffectsDialog = QtGui.QDialog()
fullEffectsDialog_ui=Ui_FullEffectsDialog()
fullEffectsDialog_ui.setupUi(fullEffectsDialog)
setUpFullEffectDialog(fullEffectsDialog_ui)
